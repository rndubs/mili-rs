...
| `milox` M1–M4 parity           | 262 | `import milox` vs upstream `mili`. M1 metadata (171) + M2 `nodes()`/`connectivity()` (51) + M3 primal `query()` (17) + **M4 Slice A** filter-combination table + `test_bugfixes` Slice-A values + **M4-followup Slice B**: bare-component-of-VEC_ARRAY + ip-as-label + `f"{comp} ipt. {label}"` naming + cross-material `InconsistentIpCounts` (`test_bugfixes_slice_b_oracle` ×4, `_component_names`, `_cross_material_inconsistent_ips_contract`) — all bit-exact, no xfail. Dedicated `test-milox` CI job; `mili-py` excluded from default `cargo test --workspace` |
