...
- **Duplicate snames within a directory type — the last un-exercised
  ...
  no duplicate snames within a directory type. A whole-corpus audit
  (155 A-files under `reference/mili-python/tests/data/**` +
  `reference/mili/test/xmilics/**`) found **57 fixtures that do** —
  ELEM_CONNS ×53, CLASS_DEF ×4, none in d3samp6. Upstream `AFileWriter`
  *merges* same-sname entries at parse (`np.concatenate`) and writes
  one payload per sname, updating only the **first** matching decl's
  offset/length/strings while every later duplicate keeps a **stale**
  original offset/length yet is still emitted — so the raw-copy path
  would silently emit wrong bytes for those 57. Reproduced exactly in
  `mili_rs::write` (`DIR_ORDER` loop now group-by-sname; single-entry
  groups raw-copy unchanged so the no-dup corpus is byte-identical;
  ELEM_CONNS duplicates merged as `superclass ‖ Σblock_cnt ‖ blocks*
  ‖ conns*`; CLASS_DEF empty; any other type a hard error — never a
  silent wrong write). Gated by `parity_write_dup_sname.rs` (5
  fixtures byte-diffed vs the upstream golden). Phase 3 has no
  remaining un-gated writer path.
...
- **Closeout redirect-coverage audit surfaced one latent milox
  ...
  modules into the harness was bit-clean except `test_bugfixes ::
  Bugfixes0_2_5.test_srec_offsets`, which compares
  `db._mili.subrecords()[i].state_byte_offset` against a committed
  Griz oracle (`vecarray/shell_mat15/srec-offsets.txt`). milox returned
  the dataclass default sentinel **`-1`** for every subrecord — the
  `subrecords()` FFI tuple simply never carried the field; no prior
  redirect test read it. Upstream computes it once during parse
  (`miliinternal.py:235,271-272`) as a running cumulative offset over
  the flattened subrecord list, `state_byte_offset = offset; offset +=
  srec.byte_size` with `byte_size = total_ordinal_count * Σ(svar
  byte sizes)`. Fixed in the Rust core
  (`mili_rs::reshape::subrecords` — `running` accumulator over
  `Subrecord::object_count() * derive_lumps(...).bytes_per_object()`,
  reusing the existing query-layer `atoms_and_widths` made
  `pub(crate)`; the byte-for-byte analogue of `collect_matching_subrecs`'s
  per-subrec `running`), threaded through the `subrecords()` /
  `subrecords_per_fragment()` FFI tuple and `milox._MiliInternal`. Pure
  metadata resolution, no value math (decision-19-compliant). Same
  class of latent read-path completeness fix as decision 24's three —
  the audit's value is exactly this: exercising the last unredirected
  surface flushes the last latent gaps before locking the gate.
- **Phase 3.3 surfaced three latent milox read-path gaps, none in
  `AppendStatesTool` itself** (`m4.md` decision 24). The tool ported
  verbatim and bit-clean on the *first* run of every code path it
  introduced; what failed were three pre-existing read bugs the prior
  redirect suites never exercised: (1) `_MiliInternal.state_variables()`
  built flat `StateVariable`s with `svars=[]`, so `atom_qty` (a
  `sum(... for sv in svars)`) was **0** for every VECTOR/VEC_ARRAY —
  the tool's data-shape validation is the first caller to read
  `svar_def.atom_qty`; fixed by porting upstream's recursive
  `svars[]` population (`miliinternal.py:140-146`). (2) A directly-
  queried element-set VEC_ARRAY (`query("es_3c","shell",ips=[2])`)
  treated `ips=` as a **0-based position**, but upstream interprets it
  as an integration-point *label* mapped via `.index(ip)` against the
  element-set payload (`miliinternal.py:191,1251-1270`) — so a valid
  request raised `IpOutOfRange`, and `ips=[1]` *silently selected the
  wrong IP* (position 1, upstream label 1 = position 0); the
  bare-component substitution path was already label-correct, only the
  direct-name path was wrong. `query_fixtures.rs ::
  d3samp4_vec_array_ip_filter_…` had **encoded the wrong positional
  contract** (a pre-oracle self-consistency test; `ips=[0]` is invalid
  upstream — "0 not in list [1,2]") and was corrected to the IP labels
  `[1,2]`. (3) A state-less `copy_non_state_data` output has **no
  `<root>T` tfile** (upstream `AFileWriter` writes smaps inline with
  `allow_tfile=False`), but the family layer's `StateMapSource::pick`
  assumed v3 + `qty_states==0` ⇒ external tfile and errored on reopen;
  upstream `afileIO` only reads a tfile *if it exists on disk*, so a
  missing tfile is now a valid 0-state inline map. All read-path fixes;
  the whole milox suite stayed green (900 pass) and every Rust parity
  gate (incl. the new `parity_write_append_states_tool.rs`) is bit-exact.

- **The Phase-3.1 "serial-sstate multi-entry-per-sname dir merge"
  blocker did not exist; the real append-to-existing blocker was a
  stale in-memory model + a live-mmap clobber** (Phase 3.2,
  `mili-rs::write.rs` / `family.rs::reload`, decision 23). Empirically
  `serial/sstate/d3samp6.plt`'s `.A` has **no** duplicate snames in
  any directory type and `copy_non_state_data` / `append_state` were
  already byte-identical to the upstream `AFileWriter` on it (verified
  `cmp`-equal). The 14 honest-xfailed `test_append_states` cases
  failed because they re-query the **same** db object after
  `append_state`, but milox's engine is the Rust core whose
  `states`/`directory`/`a_mmap` were unchanged — upstream's
  `_MiliInternal.append_state` mutates its in-memory `__smaps`. Fix:
  FFI `append_state` is `&mut self` + `Database::reload` (re-parse the
  just-rewritten `.A`). That surfaced the *actual* "NODES: payload
  shorter than declared body" (the symptom the Phase-3.1 note
  guessed at): `std::fs::write` truncates the `.A` **in place under
  the database's own live `MAP_SHARED` mapping**, corrupting the
  in-call nodal read and the in-process re-parse. Fixed by an
  **atomic write-then-rename** (new inode; the old mapping keeps the
  original bytes). Also a latent Phase-3.1 bug the redirected suite
  caught: `append_state(zero_out=True, n>0)` must copy the
  **previous state's** nodpos (upstream queries `states=[prev]`,
  `miliinternal.py:1527-1531`), not the initial mesh coords. No
  corpus-vs-doc byte bug — the bytes were always right; the model
  staleness + the mmap-write hazard were the story. Found doing
  Phase 3.2.
- **Upstream `_MiliInternal.append_state` is implemented *in terms
  of* `query(write_data=)`** (Phase 3.2, decision 23). The nodpos /
  sand patch on a zeroed / first state (`miliinternal.py:1518-1538`)
  is literally `self.query("nodpos"/"sand", …, write_data=…)`. So the
  Phase-3.2 scatter is not just `modify_database`'s surface — it is
  the primitive Phase 3.1's state-file patch *should* have been; the
  core now expresses both through the one `ReadPlan`-inverting
  `scatter_query` / `scatter_state_field` path. Found doing Phase 3.2.
- **Upstream `mili.afileIO` parse→write is a *renormalisation*, not
  a byte-identity round-trip — but every payload except
  `STATE_VAR_DICT` is still a verbatim original-`.A`-range copy**
  (Phase 3.1, `crates/mili-rs/src/write.rs`, decision 22). Measured
  before implementing the writer: `AFileWriter.write(AFileParser.
  parse(X))` differs from `X` at byte 24 — upstream reorders
  directories into a fixed `dir_order`, rebuilds the string pool in
  write order, recomputes offsets/lengths/dir-decls, rebuilds the
  `STATE_VAR_DICT` int/char stream, and rewrites the smap block +
  footer. So the Rust writer must reproduce upstream's *output*, not
  the original file. The bounding empirical finding (every d3samp6
  fragment): grouping original directory payloads by `(type,
  strings)` in first-occurrence order, **42/42 non-`STATE_VAR_DICT`
  directories + `SREC_DATA` are emitted byte-identical to the
  concatenated original mmap byte-ranges**, and the 16-byte header is
  a verbatim copy — only `STATE_VAR_DICT` genuinely diverges (the C
  writer's int stream ≠ upstream's `__collect_svar_data`
  reconstruction at payload byte 160). This is what makes the
  Rust-core writer bounded (copy raw ranges; rebuild only the svar
  dict + string pool + dir-decls + smaps + footer) and is gated
  bit-exact by `parity_write_append.rs`. Also: `copy_non_state_data`
  resets the `state_count` APPLICATION_PARAM scalar to **0**
  (`datatypes.py:546-547`) and `append_state` then increments it to
  1 (`miliinternal.py:1494`) — the Rust writer special-cases that one
  4-byte payload instead of raw-copying it. No corpus-vs-doc bug;
  the format docs already described the trailer/state-map layout. The
  d3samp6 corpus has no duplicate snames within a directory type, so
  upstream's stale-offset-on-duplicate-sname quirk
  (`__write_directories` updates only the first decl per sname while
  `__write_dir_decls` emits all) does not arise — flagged in
  `phase-3.md` for a future corpus that has it. Found doing Phase 3.1.
- **I.4 decision 21 superseded the I.3 wrapper mechanism: milox's
  parallel handler is now upstream's *exact* per-proc algorithm, not
  the Rust merge** (`milox/parallel.py`, `milidatabase.py`,
  `adjacency.py`). The `LoopWrapper`/`ServerWrapper` hold a real
  per-proc list of `_MiliInternal` (each `open_single` over one
  fragment's A-file — serial-gate bit-exact) and `MiliDatabase`
  applies upstream's per-method `reduce_function` table over the
  verbatim `milox.reductions`; `adjacency.py` is the verbatim upstream
  port over the per-proc `geometry` rewrap. This is bit-exact **by
  construction** wherever upstream is — *including* the `db0()`-only
  accessors the I.3 Rust-merge path could not reproduce (the prior
  surprise's honest-xfail boundary is gone — all promoted). The Rust
  `DatabaseSet` merge is unchanged and still backs the direct
  `_MiliInternal(set_db)` consumers (e.g. `test_miliinternal` opening a
  parallel base); only the *wrapper* path changed. Net `merge_results`
  values unchanged where the Rust merge was already bit-exact (the 564
  held). Found doing Phase I.4 — the I.3 decision-point ("keep the
  Rust merge, don't double-work") was the right *incremental* call but
  I.4's explicit job was to take the faithful contract the rest of the
  way.
- **upstream `_MiliInternal.query` never raises — every failure is an
  empty result + ERROR return code** (`milox/miliinternal.py`,
  Phase I.4). milox's Rust core *raises* `MiliPythonError` when a
  svar/class is absent; in a parallel db a fragment routinely lacks a
  class, so the wrapper's per-proc fan-out would abort on the first
  such fragment. Upstream `miliinternal.py:__query` instead returns a
  well-formed but empty `res` keyed by the queried svars and sets the
  ERROR return code (`parse_return_codes` then raises only when *all*
  procs error / the serial case). milox now catches the Rust error and
  mirrors that empty-res+code shape — a corpus/oracle-wins fit of the
  Rust exception surface onto upstream's return-code contract. Also:
  `_MiliInternal.material_numbers` returned the Rust `list` but
  upstream returns `np.array(...)` — fixed to ndarray (surfaced only
  once the parallel reduce correctly produced an ndarray). Found doing
  Phase I.4.
- **milox `merge_results=True` is the Rust `DatabaseSet` merge wearing
  the `_MiliInternal` shape — not a Python re-merge** (Phase I.3,
  superseded by decision 21 above for the wrapper path; still true for
  the direct `_MiliInternal(set_db)` path). The
  `LoopWrapper`/`ServerWrapper`
  `merge_results=True` arm forwards every read to `_MiliInternal(db)`
  over the Set-backed `PyMiliDatabase`; the Rust core already did
  upstream's per-fragment reduction bit-exactly (decision 19) and
  `_MiliInternal` only reshapes it to the upstream accessor
  signature/return type. `reductions.combine` stays identity over the
  single merged dict. The decision-point boundary: where the Rust
  merged accessor is genuinely cross-fragment (labels / connectivity /
  nodes / times / the svar-table reshapes) it is bit-exact and the
  wrapper tests promoted; where the Set accessor is **`db0()`
  fragment-0-only** (not a true merge:
  `class_labels_of_material` / `materials_of_class_name` /
  `parts_of_class_name` / `nodes_of_material` / `nodes_of_elems` /
  `int_points_of_state_variable` / `queriable_svars` /
  `state_variable_titles` / `classes_of_*` / `all_labels_of_material`)
  it differs from upstream's per-proc `_MiliInternal` + `reduce_<X>`
  and is honest strict-xfail → the I.4 per-proc-list + reduce path.
  Corollary: **`milox.reductions` was already a complete verbatim
  port** before I.3 (it landed in the Phase-H reductions sub-slice),
  contradicting the pre-I.3 plan text that called it "currently
  identity" — a state-vs-doc drift; the doc was the stale party, the
  code won. Found doing Phase I.3.
- **`database.rs::superclass_from_class_name` must scan fragments,
  not `db0()`** — a class is declared only on the MPI ranks that own
  elements of it (d3samp6 `beam` is absent from fragment 0), so the
  fragment-0-only lookup wrongly returned "does not exist" once
  `_MiliInternal.connectivity`/`int_points_of_state_variable` began
  guarding on it (Phase I.3). Fixed to first-hit-wins across
  fragments, **exactly** upstream
  `reductions.reduce_superclass_from_class_names`
  (reductions.py:143-148: first non-`M_INVALID_LABEL` across procs).
  Decision-19-compliant (metadata resolution, no value math). Found
  by the `test_connectivity_by_class[xmilics-d3samp6]` regression
  during Phase I.3.
