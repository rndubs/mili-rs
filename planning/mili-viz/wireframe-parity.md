...
| Results → `derived` | 🟡 partial | **stays** the hard-coded 7-name representative `DERIVED_RESULTS` (openly labelled). A faithful derived catalog is DB-filtered (griz iterates `analy->derived_results`; mili-python `supported_derived_variables`/`derived_variables_of_class`) and needs a `mili-rs` **core** derived registry + eligibility port with a `mili` oracle — a re-port, not the `queriable_svars` reshape (`phase-5-m4.md` Decision 70; blocker: no `supported_derived_variables`/`derived_variables_of_class` in `mili-rs`/`mili-py` — an architecturally-significant separate core milestone). Reserved blob tag space keeps it a zero-proto-change forward seam | phase-5-m3 Dec 47 / phase-5-m4 Dec 70 |
