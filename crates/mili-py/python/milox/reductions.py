"""milox ``reductions`` — verbatim port of upstream
``mili.reductions``.

Pure post-processing over the already-parity-correct primal/derived
``QueryDict`` (decision 18): merge / concatenate / unique helpers and
``combine``. Zero parity risk — the Rust ``DatabaseSet`` has already
performed the per-fragment merge before results cross the FFI boundary,
so for the serial / single-fragment case ``combine`` receives (and
returns) a single dict unchanged; the list-merge helpers are kept
byte-identical to upstream so the redirected ``test_reductions``
collects and the ``MiliDatabase`` modifier path matches exactly.

Ported essentially line-for-line from
``reference/mili-python/src/mili/reductions.py`` (only the ``mili.*``
imports are repointed at ``milox``).
"""
...
from .datatypes import QueryDict, QueryLayout, Superclass
...
def dictionary_merge_concat(
    dictionaries: List[Dict[KT, NDArray[Any]]], axis: Optional[int] = None
) -> Dict[KT, NDArray[Any]]:
    ...
    for dictionary in dictionaries:
        for key, value in dictionary.items():
            if key not in combined_dictionary:
                ...
                combined_dictionary[key] = np.append(
                    combined_dictionary[key], value, axis=axis
                )
    return combined_dictionary


def dictionary_merge_concat_unique(
    dictionaries: List[Dict[KT, NDArray[Any]]], axis: Optional[int] = None
) -> Dict[KT, NDArray[Any]]:
...
def merge_result_dictionaries(
    result_dicts: List[Dict[str, QueryDict]],
) -> Dict[str, QueryDict]:
    ...
    for svar in svars:
        ...
        processors_with_data = [
            result_dicts[i]
            for i in np.where([d[svar]["data"].size > 0 for d in result_dicts])[0]
        ]
            ...
            merged_results[svar] = QueryDict(
                ...
                layout=QueryLayout(
                    ...
                    labels=np.concatenate(
                        [d[svar]["layout"]["labels"] for d in processors_with_data]
                    ),
                ),
                data=np.concatenate(
                    [d[svar]["data"][:, :, :] for d in processors_with_data], axis=1
                ),
                modifier=processors_with_data[0][svar].get("modifier", ""),
            )

            _, indexes, counts = np.unique(
                merged_results[svar]["layout"]["labels"],
                return_counts=True,
                return_index=True,
            )
            if np.any(counts > 1):
                merged_results[svar]["data"] = merged_results[svar]["data"][
                    :, indexes, :
                ]
                merged_results[svar]["layout"]["labels"] = merged_results[svar][
                    "layout"
                ]["labels"][indexes]
...
@overload
def combine(
    parallel_results: Union[Dict[str, QueryDict], List[Dict[str, QueryDict]]],
) -> Dict[str, QueryDict]: ...
@overload
def combine(
    parallel_results: Union[
        Dict[str, pd.DataFrame], List[Dict[str, pd.DataFrame]]
    ],
) -> Dict[str, pd.DataFrame]: ...
...
def combine(
    ...
    if isinstance(parallel_results, list) and len(parallel_results) > 0:
        ...
        if isinstance(list(parallel_results[0].values())[0], dict):
            return merge_result_dictionaries(parallel_results)  # type: ignore
        if isinstance(list(parallel_results[0].values())[0], pd.DataFrame):
            return merge_dataframes(parallel_results)  # type: ignore

    raise ValueError("An error occurred while trying to combine")
...
def list_concatenate_unique(
    l: Union[List[List[Any]], NDArray[Any]],
) -> NDArray[Any]:
    """Concatenate list and remove duplicate values"""
    ...
    if len(l) == 0:
        ...
        return pd.unique(np.concatenate(l))  # type: ignore
...
def reduce_nodes_of_elems(
    parallel_results: List[Tuple[NDArray[np.int32], NDArray[np.int32]]],
    ...
    return np.concatenate(combined_nodes, dtype=np.int32), np.concatenate(
        combined_elems, dtype=np.int32
    )


def reduce_labels(
    parallel_results: Union[
        List[NDArray[np.int32]], List[Dict[str, NDArray[np.int32]]]
    ],
) -> Union[NDArray[np.int32], Dict[str, NDArray[np.int32]]]:
    """Merge the results of the labels method into the serial format"""
    if isinstance(parallel_results[0], dict):
        return dictionary_merge_concat_unique(parallel_results)  # type: ignore
    else:
        return list_concatenate_unique(parallel_results)  # type: ignore
...
def reduce_connectivity(
    ...
    if isinstance(parallel_results[0], dict):
        return dictionary_merge_concat(parallel_results, axis=0)  # type: ignore
    else:
        return list_concatenate(parallel_results)  # type: ignore
