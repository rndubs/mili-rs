...
Verbatim port of upstream ``mili.projection``
(``reference/mili-python/src/mili/projection.py``). Decision-18/19
precedent (the ``reductions`` / ``utils`` / GeometricMeshInfo /
derived-listing ports): the projection routines are pure numpy
gather + accumulate + divide over inputs that are *already*
parity-correct in the Rust core — ``query['data']`` (the core primal /
derived query), ``nodes_of_elems`` (``mili_rs::geometry``,
``parity_geometry.rs``), ``superclass_from_class_name``
(``mili_rs::reshape``) and the ``element_volume`` derived
(``mili_rs::derived``/``geometry``). Running the *same* numpy code on
those bit-exact inputs reproduces upstream bit-for-bit by construction;
there is no new numerical kernel to push into core (contrast the
derived value engine, which computes new values from raw primals and so
lives in ``mili_rs::derived`` with ``parity_*.rs`` coverage). The
parity gate is the redirected upstream ``test_projection.py``
(hard-coded mili-python oracle values), exactly as ``test_derived.py``
gates the derived sub-slices.
...
from .datatypes import QueryDict, Superclass
if TYPE_CHECKING:
