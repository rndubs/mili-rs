...
Verbatim port of upstream ``mili.append_states``
(``reference/mili-python/src/mili/append_states.py``, Phase 3.3 /
decision 24 in ``planning/mili-py/m4.md``). Decision-18/19 precedent
(the ``reductions`` / ``utils`` / ``projection`` / ``adjacency``
ports): ``AppendStatesTool`` is pure input-spec validation +
orchestration — it drives the *already* bit-exact Rust-core write
primitives ``append_state`` / ``copy_non_state_data`` (Phase 3.1,
``parity_write_append.rs``) and ``query(write_data=)`` (Phase 3.2,
``parity_write_query.rs``) over the parity-correct read accessors
(``state_variables`` / ``classes_of_state_variable`` /
``containing_state_variables_of_class`` / ``int_points_of_state_variable``
/ ``labels`` / ``times``). Running the *same* validation + numpy
reshape on those bit-exact primitives reproduces upstream by
construction; there is no new byte-layout kernel to push into the core
(contrast the writer itself, decision 22). Ported line-for-line from
upstream (only the ``mili.*`` imports repointed at ``milox``); the
behavioural parity gate is the redirected upstream
``test_append_states_tool.py``.

SPDX-License-Identifier: (MIT)
"""

import numpy as np
from . import reader, datatypes
from typing import *

from .datatypes import StateVariable
