"""milox ``grizinterface`` — verbatim port of upstream
``reference/mili-python/src/mili/grizinterface.py`` (213 LOC).

Only the module imports are rebased onto the milox package
(``mili.*`` → ``.``); the ``GrizInterface`` / ``open_griz_interface``
bodies are byte-for-byte upstream. ``GrizInterface.__init__`` is built
entirely around the ``merge_results=False`` per-proc contract — it is
served by the Phase-I.2 ``LoopWrapper`` / ``ServerWrapper`` per-proc
forwarding over the Phase-I.1 ``*_per_fragment()`` accessors (the
parallel fan-out itself stays in the Rust ``DatabaseSet``; this is
non-parity plumbing over already-parity-correct per-fragment outputs —
decision 18/20).
"""
...
from .reader import *
from .afileIO import MiliAParseError, MiliFileNotFoundError
